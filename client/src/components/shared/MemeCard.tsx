...
import { MemeData } from "../../_root/pages/Home";
...
const MemeCard = ({ meme }: MemeCardProps) => {
  ...
  const handleLike = () => {
    likePost({ postId: meme.id, isLiked: liked }, {
  ...
  const handleSave = () => {
    savePost({ postId: meme.id, isSaved: saved }, {
      onSuccess: () => {setSaved(!saved); setToast(['Success', `Meme ${meme.id} was successfully ${saved ? 'unsaved' : 'saved'}`])},
  ...
  return (
    <div className="meme-card-container">
      ...
      {/* caption */}
      <p className="meme-card-caption">{meme.caption}</p>

      {/* if the user created the post then allow edit */}
      {meme.creator_id === user?.id && (
        <Link to={`posts/edit/${meme.id}`}>
          <EditIcon height="20px" width="20px" className="meme-card-edit" />
        </Link>
      )}

      {/* Image */}
      <Link style={{ textDecoration: "none" }} to={`/posts/${meme.id}`}>
        <img
          src={`http://localhost:5000/public/${meme.url}`}
          className="meme-card-image"
        />
      </Link>

      <span className="meme-card-span">
        {/* Likes */}
        <span style={{ cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '5px' }} onClick={handleLike}>
          <SvgIcon fill={liked ? "red" : "#76abae"} width="30px" height="30px" path={liked ? likedIcon : unlikedIcon} viewbox="0 0 24 24" />
        ...
        <span style={{ cursor: 'pointer' }} onClick={handleSave}>
          <SvgIcon fill={saved ? "#eeeeee" : "rgba(118, 171, 174, 0.3)"} width="30px" height="30px" path={savedIcon} viewbox="0 0 24 24" />
        </span>
      </span>
      ...
      <span className="meme-card-span">
        ...
        {/* Time stamp */}
        <p className="meme-card-creation">{formatDate(meme.created_at)}</p>
      </span>
    </div>
  );
};
